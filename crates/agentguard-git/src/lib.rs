//! Safe, local-only Git metadata for AgentGuard workspaces.
//!
...
use agentguard_core::{AgentGuardError, RelativeRepoPath, Result};
...
pub fn snapshot_source(repository: impl AsRef<Path>) -> Result<SourceSnapshot> {
    ...
    if !root.is_dir() {
        return Err(AgentGuardError::Git(
...
pub fn initialize_visible_baseline(workspace: impl AsRef<Path>) -> Result<String> {
    ...
    git(
        ...
        [
            ...
            "-c",
            "user.email=agentguard@local",
            "commit",
            "--quiet",
            "--no-verify",
            "-m",
            "AgentGuard visible workspace baseline",
        ],
    )?;
...
fn git<const N: usize>(root: &Path, args: [&str; N]) -> Result<String> {
    let output = Command::new("git")
        ...
        .args(args)
        .output()
        .map_err(|error| AgentGuardError::Git(format!("could not execute git: {error}")))?;
    if !output.status.success() {
        return Err(AgentGuardError::Git(format!(
            "git command failed with status {}",
            output.status
        )));
    }
    String::from_utf8(output.stdout)
        .map_err(|_| AgentGuardError::Git("git returned non-UTF-8 path data".to_owned()))
}
