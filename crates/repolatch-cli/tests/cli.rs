...
fn bin() -> Command {
    Command::cargo_bin("repolatch").expect("binary")
}
...
fn malformed_policy_fails_validation() {
    let temp = TempDir::new().expect("tempdir");
    fs::write(temp.path().join("repolatch.toml"), "version = 99").expect("policy");
    bin()
        .args(["policy", "validate", "--repo"])
        .arg(temp.path())
        .assert()
        .failure()
        .stderr(predicate::str::contains("invalid repolatch.toml"));
}
...
fn docker_e2e_isolation() {
    ...
    for name in ["repolatch.toml", "safe.txt", ".env"] {
