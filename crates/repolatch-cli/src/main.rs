...
use repolatch_core::{CommandSpec, RelativeRepoPath, RepoRoot, SessionId};
use repolatch_git::{initialize_visible_baseline, snapshot_source, summarize_workspace_diff};
...
struct ValidationOutput<'a> {
    ...
    docker: &'a repolatch_core::BackendCapabilities,
    local: &'a repolatch_core::BackendCapabilities,
...
fn validate(args: PolicyArgs) -> anyhow::Result<()> {
    ...
    let docker = DockerBackend::new("docker", "repolatch-validation")?.capabilities();
...
fn print_capabilities(name: &str, caps: &repolatch_core::BackendCapabilities) {
...
fn run_command(args: RunArgs) -> anyhow::Result<()> {
    ...
    let receipt_path = args.receipt.unwrap_or_else(|| {
        ...
        workspace
            ...
            .join(format!("{name}.repolatch-receipt.json"))
    });
