...
impl RepoRoot {
    pub fn discover(path: impl AsRef<Path>) -> Result<Self> {
        ...
        if !canonical.is_dir() {
            return Err(RepoLatchError::InvalidPath(
...
impl RelativeRepoPath {
    pub fn new(path: impl AsRef<Path>) -> Result<Self> {
        ...
        if path.as_os_str().is_empty() {
            return Err(RepoLatchError::InvalidPath("path is empty".to_owned()));
        }
...
fn os_to_utf8(part: &OsStr) -> Result<String> {
    part.to_str().map(ToOwned::to_owned).ok_or_else(|| {
        RepoLatchError::InvalidPath("path is not valid UTF-8 and cannot be receipted".to_owned())
    })
}
