...
use agentguard_core::{AgentGuardError, BackendCapabilities, RelativeRepoPath, Result, SessionId};
use agentguard_git::DiffSummary;
...
impl Receipt {
    ...
    pub fn finish(
        ...
        if !matches!(self.status, ReceiptStatus::Partial) {
            return Err(AgentGuardError::Receipt(
...
impl ReceiptWriter {
    ...
    pub fn write(&self, receipt: &Receipt) -> Result<()> {
        let bytes = serde_json::to_vec_pretty(receipt).map_err(|error| {
            AgentGuardError::Receipt(format!("could not serialize receipt: {error}"))
        })?;
        let parent = self
            .path
            .parent()
            .ok_or_else(|| AgentGuardError::Receipt("receipt path has no parent".to_owned()))?;
...
pub fn render_json(receipt: &Receipt) -> Result<String> {
    serde_json::to_string_pretty(receipt)
        .map_err(|error| AgentGuardError::Receipt(format!("could not render receipt: {error}")))
}
#[must_use]
pub fn render_terminal(receipt: &Receipt) -> String {
    format!(
        "AgentGuard receipt v{}\nsession: {}\nstatus: {:?}\nbackend: {}\nsource revision: {}\npolicy sha256: {}\nvisible paths: {}\ndenied paths: {}\nchanged files: {}\nlines: +{} -{}\nexit code: {}",
...
mod tests {
    use super::*;
    use agentguard_core::{CapabilityState, EnforcementLevel};
