//! Safe workspace construction and execution backends for RepoLatch.
//!
...
use repolatch_core::{
    BackendCapabilities, CapabilityState, CommandSpec, EnforcementLevel, RelativeRepoPath,
    RepoLatchError, RepoRoot, Result, SessionId,
};
...
///
/// Implement this over `repolatch-policy`'s compiled policy. `false` means the file is omitted.
...
impl Workspace {
    ...
    fn checked_workdir(&self, path: Option<&RelativeRepoPath>) -> Result<PathBuf> {
        ...
        if !joined.starts_with(&self.root) || !joined.is_dir() {
            return Err(RepoLatchError::Workspace(
...
impl<'a, P: VisibilityPolicy> WorkspaceBuilder<'a, P> {
    ...
    /// RepoLatch state directory; this method makes the workspace itself owner-only on Unix.
    pub fn build_at(
        &self,
        ...
        let destination_parent = destination.parent().ok_or_else(|| {
            RepoLatchError::Workspace("workspace destination has no parent".to_owned())
        })?;
    ...
    for entry in fs::read_dir(&source)? {
        ...
        if metadata.is_dir() {
            ...
            let parent = destination.parent().ok_or_else(|| {
                RepoLatchError::Workspace("invalid destination parent".to_owned())
            })?;
...
impl DockerBackend {
    ...
    pub fn argv(&self, workspace: &Workspace, request: &ExecutionRequest) -> Result<Vec<OsString>> {
        ...
        let relative_workdir = workdir
            ...
            .map_err(|_| RepoLatchError::Workspace("working directory escape".to_owned()))?;
...
impl ExecutionBackend for DockerBackend {
    ...
    fn ensure_available(&self) -> Result<()> {
        let status = Command::new(&self.executable)
            ...
            .env("PATH", SAFE_PATH)
            .output();
        match status { Ok(output) if output.status.success() => Ok(()), Ok(_) => Err(RepoLatchError::BackendUnavailable("Docker daemon is unavailable".to_owned())), Err(error) if error.kind() == io::ErrorKind::NotFound => Err(RepoLatchError::BackendUnavailable("Docker executable is unavailable; enforced execution will not fall back to local".to_owned())), Err(error) => Err(RepoLatchError::BackendUnavailable(format!("Docker availability check failed: {error}"))) }
    }
    fn execute(
        &self,
        workspace: &Workspace,
...
mod tests {
    ...
    fn environment_filters_secrets_and_docker_never_falls_back() {
        ...
        let backend = DockerBackend::new("definitely-not-repolatch-docker", "image").unwrap();
        assert!(matches!(
            backend.ensure_available(),
            Err(RepoLatchError::BackendUnavailable(_))
        ));
    }
    ...
    fn unavailable_execution_has_deterministic_partial_and_interrupted_events() {
        ...
        let backend = DockerBackend::new("repolatch-docker-not-installed", "image").unwrap();
