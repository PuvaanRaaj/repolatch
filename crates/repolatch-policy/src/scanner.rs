...
use repolatch_core::{RelativeRepoPath, RepoRoot};
...
pub struct ScanOptions {
    ...
    /// Omit paths ignored by repository ignore files while walking.
    /// Keep this disabled for security inspection so ignored secrets are still classified.
...
pub fn scan_repository(
    ...
    let mut walk_builder = WalkBuilder::new(root.as_path());
    walk_builder
        .follow_links(false)
        .hidden(false)
    ...
    for next in walker {
        ...
        let kind = if file_type.is_some_and(|kind| kind.is_file()) {
            FileKind::File
...
mod tests {
    ...
    fn adds_git_tracked_and_ignored_annotations() {
        ...
        fs::create_dir(directory.path().join("ignored-directory")).unwrap();
        fs::write(
            directory.path().join("ignored-directory/generated.txt"),
            "fake data",
        )
        .unwrap();
        ...
        assert_eq!(
            scan.entries
                .iter()
                .find(|entry| entry.path.as_str() == "ignored-directory")
                .unwrap()
                .git_ignored,
            Some(true)
        );
    ...
    #[test]
    fn repository_view_walk_can_omit_gitignored_directories() {
        let directory = tempdir().unwrap();
        fs::write(directory.path().join(".gitignore"), "generated/\n").unwrap();
        fs::create_dir(directory.path().join("generated")).unwrap();
        fs::write(
            directory.path().join("generated/large-output.txt"),
            "fake data",
        )
        .unwrap();
        fs::write(directory.path().join("visible.txt"), "fake data").unwrap();
        if Command::new("git")
            .args(["init", "-q"])
            .current_dir(directory.path())
            .status()
        ...
        assert!(
            !scan
                .entries
                .iter()
                .any(|entry| entry.path.as_str().starts_with("generated"))
        );
    }
