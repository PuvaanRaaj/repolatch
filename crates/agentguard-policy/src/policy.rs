...
/// A documented starting point for a project-level `agentguard.toml`.
pub const DEFAULT_POLICY_TEMPLATE: &str = r#"# AgentGuard policy. Deny rules always take precedence.
version = 1

[filesystem]
read = ["src/**", "tests/**", "docs/**", "Cargo.toml"]
write = ["src/**", "tests/**"]
...
impl Policy {
    ...
    pub fn parse(source: &str) -> std::result::Result<Self, PolicyError> {
        let raw: RawPolicy = toml::from_str(source)
            .map_err(|error| PolicyError(format!("invalid agentguard.toml: {error}")))?;
        if raw.version != 1 {
...
impl From<PolicyError> for AgentGuardError {
